...
static int split_pipeline(const char *line, char ***pieces){
    ...
    for (const char *p = line;; ++p){
        ...
        if (c=='\0' || (c=='|' && !in_s && !in_d)){
            ...
            rtrim(tmp); char *trim = ltrim(tmp);
            if (*trim=='\0'){
                for(size_t i=0;i<cnt;++i) free(out[i]);
                free(out); free(tmp); *pieces=NULL; return -1;
            }
            if (cnt==cap){ cap*=2; out=realloc(out,cap*sizeof(char*)); }
            out[cnt++] = strdup(trim);
            free(tmp);
            if (c=='\0') break;
            seg = p+1;
        }else if(c=='\''&&!in_d) in_s^=1;
        else if(c=='"'&&!in_s)  in_d^=1;
    }
    if (in_s||in_d){
        for(size_t i=0;i<cnt;++i) free(out[i]);
        free(out); *pieces=NULL; return -2;
    }
    ...
    *argv = malloc(cap*sizeof(char*));
    ...
    for(char *p=cmd;;++p){
        ...
        if(c=='\0' || (!in_s&&!in_d&&!esc&&isspace((unsigned char)c))){
            ...
            if(!esc && c=='\''&&!in_d){ in_s^=1; continue; }
            if(!esc && c=='"' &&!in_s){ in_d^=1; continue; }
            *o++ = c; esc=0;
        }
    }
    free(buf);
...
int main() {
    ...
    while (1) 
    {
        ...
        if (n_stages == 1 && strcmp(ltrim(stages[0]),"exit")==0){
            free_strv(stages); free(full); exit(0);
        }
        ...
        for(int i=0;i<n_stages;++i){
            int pipefd[2]={-1,-1};
            if(i<n_stages-1 && pipe(pipefd)<0){ perror("pipe"); break; }
            ...
            if(pid<0){ perror("fork"); break; }
                ...
                if(argc<0){ fprintf(stderr,"Unmatched quotes\n"); _exit(1); }
                if(argc>MAX_ARGS){ fprintf(stderr,"Too many arguments\n"); _exit(1); }
        ...
        if(prev_in!=-1) close(prev_in);
        for(int i=0;i<child_cnt;++i) waitpid(children[i],NULL,0);
